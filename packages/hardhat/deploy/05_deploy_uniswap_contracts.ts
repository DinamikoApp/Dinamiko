...
import { getChainId, getNetworkName, isLocalDevelopmentNetwork } from "../helpers/utilities/utils";
...
import { UniswapAddressNetworkConfigType } from "../helpers/types";
...
const deployUniswap: DeployFunction = async function ({ deployments, ethers }: HardhatRuntimeEnvironment) {
  ...
  if (isLocalDevelopmentNetwork(networkName)) {
    ...
    const uniswapKeys: UniswapAddressNetworkConfigType = {
    ...
    networkConfig[chainId].uniswap = uniswapKeys;
